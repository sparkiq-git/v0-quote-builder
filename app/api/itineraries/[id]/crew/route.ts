...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    for (const c of crew) {
      if (!c.role) {
        return NextResponse.json({ error: "Each crew member must have a role" }, { status: 400 })
      }
    }
    ...
    const { error: deleteError } = await supabase.from("itinerary_crew").delete().eq("itinerary_id", id)
    ...
    if (crew.length > 0) {
      ...
      const { error: insertError } = await supabase.from("itinerary_crew").insert(crewInserts)
