...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (passengers.length > itinerary.total_pax) {
      return NextResponse.json({ error: `Cannot exceed ${itinerary.total_pax} passengers` }, { status: 400 })
    }

    // Delete existing passengers for this itinerary
    const { error: deleteError } = await supabase.from("itinerary_passenger").delete().eq("itinerary_id", id)
    ...
    if (passengers.length > 0) {
      ...
      const { error: insertError } = await supabase.from("itinerary_passenger").insert(passengerInserts)
