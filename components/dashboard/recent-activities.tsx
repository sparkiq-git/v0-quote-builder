...
export function RecentActivities() {
  ...
  useEffect(() => {
    const setupRealtime = async () => {
      ...
      const addEvent = (event: ActivityEvent) => setActivities((prev) => [event, ...prev].slice(0, 50))
    ...
    <Card className="w-full h-full flex flex-col border border-border bg-card rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      ...
      <CardContent className="flex-1 p-0 overflow-hidden flex flex-col">
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full text-sm text-muted-foreground leading-relaxed">
            No recent activity.
          </div>
        ) : (
          <ScrollArea className="flex-1 overflow-y-auto">
            <div className="p-6 space-y-4">
              {activities.map((a) => (
                <div key={a.id} className="flex items-start gap-3 border-b border-border last:border-0 pb-4 last:pb-0">
                  {getIcon(a.type)}
                  <div className="flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-sm text-foreground leading-relaxed">{a.description}</span>
                      <Badge variant={getBadgeVariant(a.type)} className="text-[10px] uppercase font-medium">
                        {a.type}
                      </Badge>
                    </div>
                    <div className="text-xs text-muted-foreground mt-1 leading-relaxed">{timeAgo(a.timestamp)}</div>
                  </div>
                </div>
              ))}
            </div>
