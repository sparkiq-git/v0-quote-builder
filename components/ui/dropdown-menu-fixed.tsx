...
const DropdownMenu = ({ children, ...props }: React.ComponentPropsWithoutRef<typeof DropdownMenuPrimitive.Root>) => {
  ...
  const handleOpenChange = React.useCallback((newOpen: boolean) => {
    if (newOpen) {
      // Force synchronous layout calculation BEFORE opening
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          setOpen(true)
          // Dispatch resize to force Floating UI recalculation
...
>(({ className, sideOffset = 4, ...props }, ref) => {
  React.useLayoutEffect(() => {
    ...
    if (trigger) {
      ...
      // Multiple reflows at different intervals
...
>(({ className, inset, variant = "default", ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ...
    className={cn(
      "focus:bg-accent focus:text-accent-foreground data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 dark:data-[variant=destructive]:focus:bg-destructive/20 data-[variant=destructive]:focus:text-destructive relative flex cursor-default select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
      inset && "pl-8",
      className,
    )}
    {...props}
