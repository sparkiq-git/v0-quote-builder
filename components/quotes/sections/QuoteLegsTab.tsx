...
useEffect(() => {
  ...
  const legs = Array.isArray(quote.legs) ? quote.legs : [];
  const type = (quote.trip_type as TripType) || "one-way";

  // 🧠 Only re-sync if the trip type or number of legs has changed
  setTripType(type);
  ...
  if (type === "multi-city") {
    ...
    // ✅ Keep single-trip state in sync too
    setFormState({
      origin: legs[0]?.origin || "",
      origin_code: legs[0]?.origin_code || "",
      destination: legs[0]?.destination || "",
...
}, [quote?.id, quote?.trip_type, quote?.legs?.length]);
